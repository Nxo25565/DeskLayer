...
export class SettingsManager {
    ...
    public removeShortcut(shortcut: Shortcut){
        if (this._shortcutSettings){
            ...
            const index = this._shortcutSettings.shortcuts.findIndex(
                s => isShortcutEqual(s, shortcut)
            )
            if (index !== -1){
    ...
    async checkFiles(){
        console.log(`
            existence:
             ...
             `)
        if (!existsSync(baseSettingsPath)) { await mkdir(baseSettingsPath) }
